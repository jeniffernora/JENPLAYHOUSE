...
function renderUpdates(filter=activeUpdateFilter){
  ...
  if(authors){
    if(filter==='j-team'){
      ...
      const seen=new Set();
      const activeUsers=(data.Users||[]).filter(a=>{
        const status=String(a.Status||'active').toLowerCase();
        return status==='active' || status==='';
      });

      let list=activeUsers.filter(a=>{
        const id=String(a['User ID']||'');
        const role=String(a.Role||a.Position||'');
        return /^JN-/i.test(id) || /^JT-/i.test(id) || /artist|j-team/i.test(role);
      });

      if(filter==='artist'){
        list=list.filter(a=>{
          const id=String(a['User ID']||'');
          return /^JN-/i.test(id) || /artist/i.test(String(a.Role||a.Position||''));
        });
      }

      list=list.filter(a=>!seen.has(a['User ID'])&&seen.add(a['User ID']));

      const allBubble=filter==='all'
        ? `<button type="button" class="update-author-bubble update-author-all active" data-author-filter="all"><span class="update-author-all-mark">J</span><span class="update-author-name">All</span></button>`
        : '';

      authors.innerHTML=allBubble+list.map(a=>`<button type="button" class="update-author-bubble" data-author-filter="${escape(a['User ID'])}"><img src="${escape(img(a.Photo))}" alt="" loading="lazy" decoding="async"><span class="update-author-name">${escape(a['Display Name']||a.Name)}${updateVerified(a)}</span></button>`).join('');
    }
  }
  if(memberFilters){
    if(filter==='j-team'){
      const members=(data.Users||[]).filter(a=>{const status=String(a.Status||'active').toLowerCase();return (status==='active'||status==='')&&String(a['User ID']||'').startsWith('JT-')});
...
async function shareUpdateToX(id){
  const r=(data.Updates||[]).find(x=>x['Update ID']===id);
  if(!r){
    alert('Update not found.');
    return;
  }

  if(!window.JenUpdateCardExport || typeof window.JenUpdateCardExport.export!=='function'){
    console.error('JenUpdateCardExport is not loaded.');
    alert('Share Image exporter failed to load. Please refresh the page.');
    return;
  }

  const a=getUpdateAuthor(r['Author ID']);
  const exportData={
    ...r,
    Author:a,
    'Author Name':a.Name||a['Display Name']||'Jeniffer Nora',
    'Display Name':a['Display Name']||a.Name||'Jeniffer Nora',
    Role:a.Role||'',
    Position:a.Position||a.Role||'',
    'Author Photo':a.Photo||'',
    Avatar:a.Photo||'',
    Verified:a.Verified||'',
  ...
  try{
    await window.JenUpdateCardExport.export(exportData);
  }catch(err){
    console.error('Share Image export failed:',err);
    alert('Could not export this update image. Please try again.');
  }
}
