...
function renderCart(){const box=$('#cartItems');if(!cart.length)box.innerHTML='<p class="empty-cart-message">Your bag is empty.</p>';else box.innerHTML=cart.map((x,i)=>`<div class="cart-item"><img src="${escape(img(x.image))}"><div class="cart-item-info"><strong>${escape(x.name)}</strong><span class="cart-item-size">${escape(x.size||'')}</span><span>${money(x.price)} × ${x.qty}</span></div><button class="remove-cart" data-i="${i}">×</button></div>`).join(''); const sub=cart.reduce((s,x)=>s+x.price*x.qty,0),tax=Math.round(sub*(C.taxRate||.11)),ship=cart.length?(C.shippingFee||25000):0,total=sub+tax+ship; ['cart','checkout'].forEach(p=>{const a=$(`#${p}Subtotal`),b=$(`#${p}Tax`),c=$(`#${p}Shipping`),d=$(`#${p}Total`);if(a)a.textContent=money(sub);if(b)b.textContent=money(tax);if(c)c.textContent=money(ship);if(d)d.textContent=money(total)});$('#cartCount').textContent=cart.reduce((s,x)=>s+x.qty,0);return {sub,tax,ship,total}}
function renderNews(){const rows=visible(data.News).filter(r=>['posted','published',''].includes(String(r.Status||'').toLowerCase())).sort((a,b)=>Number(a.Order)-Number(b.Order));$('#newsList').innerHTML=rows.map(r=>`<article class="news-card" data-news="${escape(r.Slug)}"><img class="media-cover wide" src="${escape(img(r.Image))}" alt=""><p class="card-meta">${escape(r.Date)}</p><h3 class="card-title">${escape(r.Title)}</h3><p>${escape(r.Description)}</p></article>`).join('')||'<p>No news posted.</p>'}
...
function renderTeam(){const rows=visible(data['J-TEAM']).filter(r=>['posted','published',''].includes(String(r.Status||'').toLowerCase())).sort((a,b)=>Number(a.Order)-Number(b.Order));$('#teamList').innerHTML=rows.map(r=>`<article class="team-card"><img class="media-cover" src="${escape(img(r.Photo))}" alt=""><p class="card-meta">${escape(r.Position)}</p><h3 class="card-title">${escape(r.Name)}</h3></article>`).join('')||'<p>Team details coming soon.</p>'}
...
function renderUpdates(filter=activeUpdateFilter){
  ...
  if(authors){
    if(filter==='j-team'){
      ...
      authors.innerHTML=list.filter(a=>!seen.has(a['User ID'])&&seen.add(a['User ID'])).map(a=>`<button type="button" class="update-author-bubble" data-author-filter="${escape(a['User ID'])}"><img src="${escape(img(a.Photo))}" alt=""><span class="update-author-name">${escape(a['Display Name']||a.Name)}${updateVerified(a)}</span></button>`).join('');
    }
  }
  if(memberFilters){
    if(filter==='j-team'){
      ...
      memberFilters.innerHTML=`<button class="jteam-member-filter jteam-member-bubble active" type="button" data-jteam-author="all"><span class="jteam-all-avatar">J</span><span class="jteam-member-name">All</span></button>`+members.map(a=>`<button class="jteam-member-filter jteam-member-bubble" type="button" data-jteam-author="${escape(a['User ID'])}"><img src="${escape(img(a.Photo))}" alt="${escape(a['Display Name']||a.Name||'J-Team')}"><span class="jteam-member-name">${escape(a['Display Name']||String(a.Name||'').split(/\s+/)[0])}${updateVerified(a)}</span></button>`).join('');
    }else{
...
function renderHomeUpdateProfiles(){
  ...
  const profile=(u,active=false)=>`<button type="button" class="home-profile-bubble${active?' active':''}" data-home-update-author="${escape(u['User ID']||'')}"><img src="${escape(img(u.Photo||''))}" alt=""><span>${escape(u['Display Name']||u.Name||'')}</span>${String(u.Verified||'').toLowerCase()==='yes'?'<i>✓</i>':''}</button>`;
...
function renderHomeUpdatesOnly(){
  ...
  target.innerHTML=rows.map(r=>{const a=getUpdateAuthor(r['Author ID']);const media=[r['Media 1'],r['Media 2'],r['Media 3'],r['Media 4']].filter(Boolean)[0];const type=String(r['Post Type']||'').toLowerCase();return `<article class="home-update-card"><div class="home-update-head"><img src="${escape(img(a.Photo))}" alt=""><div><strong>${escape(a['Display Name']||a.Name)}${updateVerified(a)}</strong><span>${escape(r.Date||'')} ${escape(r.Time||'')}</span></div></div>${r.Text?`<p>${escape(r.Text)}</p>`:''}${media&&['photo','carousel'].includes(type)?`<img class="home-update-media" src="${escape(img(media))}" alt="">`:''}<a href="#updates" class="home-card-link">Open update →</a></article>`}).join('')||'<p class="loading-text">No updates posted yet.</p>';
...
function renderHomePreview(){
  ...
  if(newsTarget){
    ...
    newsTarget.innerHTML=rows.map(r=>`<article class="home-news-card" data-news="${escape(r.Slug)}">${r.Image?`<img src="${escape(img(r.Image))}" alt="">`:''}<div><span>${escape(r.Date||'')}</span><strong>${escape(r.Title||'News')}</strong></div></article>`).join('')||'<p class="loading-text">No news posted.</p>';
  }
  if(scheduleTarget){
